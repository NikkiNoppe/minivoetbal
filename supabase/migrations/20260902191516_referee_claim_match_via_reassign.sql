-- Andere scheids mag wedstrijd claimen door zichzelf als referee te zetten
-- (last-minute overname), ook als die nog niet toegewezen was.
...
BEGIN
  ...
  -- Resolve referee sync early (needed for claim-check)
  ...
  ELSIF v_role = 'referee' THEN
    ...
    -- Claim: niet-toegewezen scheids mag overnemen door zichzelf te zetten
    IF NOT v_can_update AND v_sync_assigned AND v_new_assigned_id = v_user_id THEN
      v_can_update := true;
    END IF;
  END IF;
